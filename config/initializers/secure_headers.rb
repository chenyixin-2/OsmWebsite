csp_policy = {
  ...
  :frame_src => %w['self'],
  :img_src => %w['self' data: localhost www.gravatar.com *.wp.com tile.openstreetmap.org *.tile.openstreetmap.org *.tile.thunderforest.com tileserver.memomaps.de *.openstreetmap.fr],
  :manifest_src => %w['self'],
  :media_src => %w['none'],
  :object_src => %w['self'],
  :plugin_types => %w[],
